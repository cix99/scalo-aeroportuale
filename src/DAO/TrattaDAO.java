...
public class TrattaDAO extends JDBC {
	...
	public int store(Tratta tratta){
        String query = "INSERT INTO " + tableName + " (destinazione, compagnia_aerea, ora_inizio_imbarco_stimato, "
        			+ "ora_fine_imbarco_stimato, max_prenotazioni, gate) VALUES  (?, ?, ?, ?, ?, ?) RETURNING (id)";
        int id = 0;
        try {
	...
	public Map<String, Map<String, Integer>> statistiche() {
	    String query = "SELECT gate.nome_gate as gate, giornaliero.voli as voli_giornalieri, giornaliero.utilizzo_stimato as s_giornaliero, "
	    		+ "giornaliero.utilizzo_effettivo as e_giornaliero, settimanale.voli as voli_settimanali, "
	    		+ "settimanale.utilizzo_stimato as s_settimanale, settimanale.utilizzo_effettivo as e_settimanale, "
	    		+ "mensile.voli as voli_mensili, mensile.utilizzo_stimato as s_mensile, mensile.utilizzo_effettivo as e_mensile "
	    		+ "FROM scalo_aeroportuale.gate LEFT JOIN( SELECT gate, COUNT(id) as voli, "
	    		+ "SUM( DATE_PART('day', tratta.ora_fine_imbarco_stimato - tratta.ora_inizio_imbarco_stimato) * 24 * 60 "
	    		+ "+ DATE_PART('hour', tratta.ora_fine_imbarco_stimato - tratta.ora_inizio_imbarco_stimato) * 60 "
	    		+ "+ DATE_PART('minute', tratta.ora_fine_imbarco_stimato - tratta.ora_inizio_imbarco_stimato)) AS utilizzo_stimato, "
	    		+ "SUM( DATE_PART('day', tratta.ora_fine_imbarco_effettivo - tratta.ora_inizio_imbarco_effettivo) * 24 * 60 "
	    		+ "+ DATE_PART('hour', tratta.ora_fine_imbarco_effettivo - tratta.ora_inizio_imbarco_effettivo) * 60 "
	    		+ "+ DATE_PART('minute', tratta.ora_fine_imbarco_effettivo - tratta.ora_inizio_imbarco_effettivo) ) AS utilizzo_effettivo "
	    		+ "FROM scalo_aeroportuale.tratta WHERE ora_inizio_imbarco_stimato BETWEEN (now() - interval '1 day') AND (now()) GROUP BY gate ) "
	    		+ "AS giornaliero ON giornaliero.gate = gate.nome_gate "
	    		+ "LEFT JOIN ( SELECT gate, COUNT(id) as voli, SUM( DATE_PART('day', tratta.ora_fine_imbarco_stimato - tratta.ora_inizio_imbarco_stimato) * 24 * 60 "
	    		+ "+ DATE_PART('hour', tratta.ora_fine_imbarco_stimato - tratta.ora_inizio_imbarco_stimato) * 60 "
	    		+ "+ DATE_PART('minute', tratta.ora_fine_imbarco_stimato - tratta.ora_inizio_imbarco_stimato) ) AS utilizzo_stimato, "
	    		+ "SUM( DATE_PART('day', tratta.ora_fine_imbarco_effettivo - tratta.ora_inizio_imbarco_effettivo) * 24 * 60 "
	    		+ "+ DATE_PART('Hour', tratta.ora_fine_imbarco_effettivo - tratta.ora_inizio_imbarco_effettivo) * 60 "
	    		+ "+ DATE_PART('minute', tratta.ora_fine_imbarco_effettivo - tratta.ora_inizio_imbarco_effettivo) ) AS utilizzo_effettivo "
	    		+ "FROM scalo_aeroportuale.tratta WHERE ora_inizio_imbarco_stimato BETWEEN (now() - interval '1 week') AND (now()) GROUP BY gate ) "
	    		+ "AS settimanale ON settimanale.gate = gate.nome_gate "
	    		+ "LEFT JOIN ( SELECT gate, COUNT(id) as voli, SUM( DATE_PART('day', tratta.ora_fine_imbarco_stimato - tratta.ora_inizio_imbarco_stimato) * 24 * 60 "
	    		+ "+ DATE_PART('hour', tratta.ora_fine_imbarco_stimato - tratta.ora_inizio_imbarco_stimato) * 60 "
	    		+ "+ DATE_PART('minute', tratta.ora_fine_imbarco_stimato - tratta.ora_inizio_imbarco_stimato) ) AS utilizzo_stimato, "
	    		+ "SUM( DATE_PART('day', tratta.ora_fine_imbarco_effettivo - tratta.ora_inizio_imbarco_effettivo) * 24 * 60 "
	    		+ "+ DATE_PART('hour', tratta.ora_fine_imbarco_effettivo - tratta.ora_inizio_imbarco_effettivo) * 60 "
	    		+ "+ DATE_PART('minute', tratta.ora_fine_imbarco_effettivo - tratta.ora_inizio_imbarco_effettivo) ) AS utilizzo_effettivo "
	    		+ "FROM scalo_aeroportuale.tratta WHERE ora_inizio_imbarco_stimato BETWEEN (now() - interval '1 month') AND (now()) GROUP BY gate ) "
	    		+ "AS mensile ON mensile.gate = gate.nome_gate";
