...
public class TrattaDAO extends JDBC {
	...
	public Map<String, Map<String, Integer>> statistiche() {
	    String query = "SELECT tratta.gate, SUM(giornaliero.voli) as voli_giornalieri, SUM(giornaliero.utilizzo_stimato) as s_giornaliero, "
	    		+ "SUM(giornaliero.utilizzo_effettivo) as e_giornaliero, SUM(settimanale.voli) as voli_settimanali, "
	    		+ "SUM(settimanale.utilizzo_stimato) as s_settimanale, SUM(settimanale.utilizzo_effettivo) as e_settimanale, "
	    		+ "SUM(mensile.voli) as voli_mensili, SUM(mensile.utilizzo_stimato) as s_mensile, SUM(mensile.utilizzo_effettivo) as e_mensile "
	    		+ "FROM scalo_aeroportuale.tratta "
	    		+ "LEFT JOIN( SELECT gate, 1 as voli, "
	    		...
	    		+ "+ DATE_PART('minute', tratta.ora_fine_imbarco_effettivo - tratta.ora_inizio_imbarco_effettivo) ) AS utilizzo_effettivo FROM scalo_aeroportuale.tratta "
	    		+ "WHERE ora_inizio_imbarco_stimato BETWEEN (now() - interval '1 day') AND (now()) GROUP BY gate ) AS giornaliero ON giornaliero.gate = tratta.gate "
	    		+ "LEFT JOIN ( SELECT gate, 1 as voli, SUM( DATE_PART('day', tratta.ora_fine_imbarco_stimato - tratta.ora_inizio_imbarco_stimato) * 24 * 60 "
	    		+ "+ DATE_PART('hour', tratta.ora_fine_imbarco_stimato - tratta.ora_inizio_imbarco_stimato) * 60 "
	    		+ "+ DATE_PART('minute', tratta.ora_fine_imbarco_stimato - tratta.ora_inizio_imbarco_stimato) ) AS utilizzo_stimato, "
	    		+ "SUM( DATE_PART('day', tratta.ora_fine_imbarco_effettivo - tratta.ora_inizio_imbarco_effettivo) * 24 * 60 "
	    		+ "+ DATE_PART('Hour', tratta.ora_fine_imbarco_effettivo - tratta.ora_inizio_imbarco_effettivo) * 60 "
	    		+ "+ DATE_PART('minute', tratta.ora_fine_imbarco_effettivo - tratta.ora_inizio_imbarco_effettivo) ) AS utilizzo_effettivo FROM scalo_aeroportuale.tratta "
	    		+ "WHERE ora_inizio_imbarco_stimato BETWEEN (now() - interval '1 week') AND (now()) GROUP BY gate ) AS settimanale ON settimanale.gate = tratta.gate "
	    		+ "LEFT JOIN ( SELECT gate, 1 as voli, SUM( DATE_PART('day', tratta.ora_fine_imbarco_stimato - tratta.ora_inizio_imbarco_stimato) * 24 * 60 "
	    		+ "+ DATE_PART('hour', tratta.ora_fine_imbarco_stimato - tratta.ora_inizio_imbarco_stimato) * 60 "
	    		+ "+ DATE_PART('minute', tratta.ora_fine_imbarco_stimato - tratta.ora_inizio_imbarco_stimato) ) AS utilizzo_stimato, "
	    		+ "SUM( DATE_PART('day', tratta.ora_fine_imbarco_effettivo - tratta.ora_inizio_imbarco_effettivo) * 24 * 60 "
	    		+ "+ DATE_PART('hour', tratta.ora_fine_imbarco_effettivo - tratta.ora_inizio_imbarco_effettivo) * 60 "
	    		+ "+ DATE_PART('minute', tratta.ora_fine_imbarco_effettivo - tratta.ora_inizio_imbarco_effettivo) ) AS utilizzo_effettivo FROM scalo_aeroportuale.tratta "
	    		+ "WHERE ora_inizio_imbarco_stimato BETWEEN (now() - interval '1 month') AND (now()) GROUP BY gate ) AS mensile ON mensile.gate = tratta.gate "
